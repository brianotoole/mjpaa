...
if ( ! function_exists( 'mjpaa_setup' ) ) :
/**
...
function mjpaa_setup() {
	...
	add_theme_support( 'custom-background', apply_filters( 'mjpaa_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif; // mjpaa_setup
add_action( 'after_setup_theme', 'mjpaa_setup' );
 ...
 */
function mjpaa_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Sidebar', 'mjpaa' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'mjpaa_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function mjpaa_scripts() {
    ...
    //if is UM page (used id 7, in case name changes), show...
    if (is_page(7) ) {
	...
	wp_enqueue_script( 'site-scripts', get_template_directory_uri() . '/js/site.min.js', array(), '1.0.0', true );
	
	//wp_enqueue_script( 'skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );

	//wp_enqueue_script( 'scripts', get_template_directory_uri() . '/js/mjpaa.scripts.js', array(), false, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		//wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'mjpaa_scripts' );
...
function mjpaa_excerpt_length( $length ) {
	...
	$excerpt = get_theme_mod('exc_length', '15');
	return $excerpt; 
